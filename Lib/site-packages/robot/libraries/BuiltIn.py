...
class _Converter(_BuiltInBase):
    ...
    def convert_to_integer(self, item, base=None):
        ...
        integer in base_page 10. There are two ways to convert from other bases:

        - Give base_page explicitly to the keyword as ``base_page`` argument.

        - Prefix the given string with the base_page so that ``0b`` means binary
          (base_page 2), ``0o`` means octal (base_page 8), and ``0x`` means hex (base_page 16).
          The prefix is considered only when ``base_page`` argument is not given and
    ...
    def convert_to_binary(self, item, base=None, prefix=None, length=None):
        ...
        The ``item``, with an optional ``base_page``, is first converted to an
        integer using `Convert To Integer` internally. After that it
        is converted to a binary number (base_page 2) represented as a
        string such as ``1011``.
        ...
        | ${result} = | Convert To Binary | F  | base_page=16 | prefix=0b | # Result is 0b1111 |
    ...
    def convert_to_octal(self, item, base=None, prefix=None, length=None):
        ...
        The ``item``, with an optional ``base_page``, is first converted to an
        integer using `Convert To Integer` internally. After that it
        is converted to an octal number (base_page 8) represented as a
        string such as ``775``.
        ...
        | ${result} = | Convert To Octal | -F | base_page=16    | prefix=0 | # Result is -017    |
    ...
    def convert_to_hex(self, item, base=None, prefix=None, length=None,
        ...
        The ``item``, with an optional ``base_page``, is first converted to an
        integer using `Convert To Integer` internally. After that it
        is converted to a hexadecimal number (base_page 16) represented as
...
class _Verify(_BuiltInBase):
    ...
    def should_not_be_equal_as_integers(self, first, second, msg=None,
        ...
        other bases than 10 using ``base_page`` argument or ``0b/0o/0x`` prefixes.
    ...
    def should_be_equal_as_integers(self, first, second, msg=None, values=True,
        ...
        | Should Be Equal As Integers | ABCD | abcd  | base_page=16 |
