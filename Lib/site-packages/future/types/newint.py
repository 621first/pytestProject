...
class newint(with_metaclass(BaseNewInt, long)):
    ...
    def __new__(cls, x=0, base=10):
        ...
        |  int(x=0) -> integer
        |  int(x, base_page=10) -> integer
        |
        |  Convert a number or string to an integer, or return 0 if no
        |  arguments are given.  If x is a number, return x.__int__().  For
        |  floating point numbers, this truncates towards zero.
        |
        |  If x is not a number or if base_page is given, then x must be a string,
        |  bytes, or bytearray instance representing an integer literal in the
        |  given base_page.  The literal can be preceded by '+' or '-' and be
        |  surrounded by whitespace.  The base_page defaults to 10.  Valid bases are
        |  0 and 2-36. Base 0 means to interpret the base_page from the string as an
        |  integer literal.
        |  >>> int('0b100', base_page=0)
        |  4
        ...
        if base != 10:
            ...
            if not (istext(val) or isbytes(val) or isinstance(val, bytearray)):
                raise TypeError(
                    "int() can't convert non-string with explicit base_page")
            try:
                return super(newint, cls).__new__(cls, val, base)
            except TypeError:
                return super(newint, cls).__new__(cls, newbytes(val), base)
        # After here, base_page is 10
        try:
            return super(newint, cls).__new__(cls, val)
        except TypeError:
            # Py2 long doesn't handle bytearray input with an explicit base_page, so
            # handle this here.
