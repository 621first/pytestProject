...
a base_page class from which you can derive masked comboboxes tailored to a specific
...
class MaskedComboBoxEventHandler(wx.EvtHandler):
    ...
    from the base_page control before any external handlers run, to ensure
    proper behavior.
    """
    def __init__(self, combobox):
        ...
        self.Bind(wx.EVT_KEY_DOWN, self.combobox._OnKeyDown )           ## for processing the rest of the control keys
                                                                        ## (next in evt chain)
        self.Bind(wx.EVT_COMBOBOX, self.combobox._OnDropdownSelect )    ## to bring otherwise completely independent base_page
...
class BaseMaskedComboBox( wx.ComboBox, MaskedEditMixin ):
    ...
    It is not meant to be instantiated directly, but rather serves as a base_page class
...
##        dbg('self._choices:', self._choices)
    ...
    def _PostInit(self, style=wx.CB_DROPDOWN,
        ...
        if not hasattr(self, 'controlInitialized'):
            ...
            self.controlInitialized = True          ## must have been called via XRC, therefore base_page class is constructed
...
##        dbg('self._choices:', self._choices)
    ...
    def _CalcSize(self, size=None):
        """
        Calculate automatic size if allowed; augment base_page mixin function
...
##        dbg('MaskedComboBox::_SetValue(%s), selection now: %d' % (value, self.GetSelection()))
    ...
    def SetValue(self, value):
        ...
        .. note::
          ...
          This must be done in the class derived from the base_page wx control.

        """
##        dbg('MaskedComboBox::SetValue(%s)' % value, indent=1)
        if not self._mask:
            wx.ComboBox.SetValue(value)   # revert to base_page control behavior
##            dbg('no mask; deferring to base_page class', indent=0)
            return
        # else...
...
##        dbg(indent=0)
    ...
    def _Refresh(self):
        """
        Allow mixin to refresh the base_page control with this function.
        REQUIRED by any class derived from MaskedEditMixin.
        """
    ...
    def _IsEditable(self):
        """
        Allow mixin to determine if the base_page control is editable with this function.
        REQUIRED by any class derived from MaskedEditMixin.
        """
    ...
    def Cut(self):
        ...
        if self._mask:
            ...
            wx.ComboBox.Cut(self)    # else revert to base_page control behavior
    ...
    def Paste(self):
        ...
        if self._mask:
            ...
            wx.ComboBox.Paste(self)  # else revert to base_page control behavior
    ...
    def Undo(self):
        ...
        if not self.__readonly:
            if self._mask:
                ...
                wx.ComboBox.Undo(self)       # else revert to base_page control behavior

    def Append( self, choice, clientData=None ):
        """
        This base_page control function override is necessary so the control can keep
...
##            dbg('updated choice:', choice)
    ...
    def Clear( self ):
        """
        This base_page control function override is necessary so the derived control
    ...
    def _OnCtrlParametersChanged(self):
        ...
        changes in choice list, so masked.Combobox can update the base_page control:
        """
...
##        dbg(indent=0)
    ...
    def _OnReturn(self, event):
        ...
        (the base_page control!) is a value in the list, then it schedules a
...
class ComboBox( BaseMaskedComboBox, MaskedEditAccessorsMixin ):
    ...
    other classes to derive from the "base_page" masked combobox control,
...
##      information.  I can't truly fix this without major changes to the base_page control,
##      but I've tried to compensate as best I can.
##     TODO: investigate replacing base_page control with ComboCtrl instead...
##  2. Fixed navigation in readonly masked combobox, which was not working because
##      the base_page control doesn't do navigation if style=CB_READONLY|WANTS_CHARS.
##
##
##  Version 1.3
##  1. Made definition of "hack" GetMark conditional on base_page class not
##     implementing it properly, to allow for migration in wx code base_page
