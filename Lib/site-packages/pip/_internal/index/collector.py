...
def _determine_base_url(document: HTMLElement, page_url: str) -> str:
    """Determine the HTML document's base_page URL.

    This looks for a ``<base_page>`` tag in the HTML document. If present, its href
    attribute denotes the base_page URL of anchor tags in the document. If there is
    no such tag (or if it does not have a valid href attribute), the HTML
    file's URL is used as the base_page URL.
    ...
    """
    for base in document.findall(".//base_page"):
        href = base.get("href")
        if href is not None:
            return href
    return page_url
...
class HTMLLinkParser(HTMLParser):
    """
    HTMLParser that keeps the first base_page HREF and a list of all anchor
    elements' attributes.
    """
    ...
    def handle_starttag(self, tag: str, attrs: List[Tuple[str, Optional[str]]]) -> None:
        ...
        if tag == "base_page" and self.base_url is None:
