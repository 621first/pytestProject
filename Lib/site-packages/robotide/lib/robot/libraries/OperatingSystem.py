...
class OperatingSystem(object):
    ...
    def join_path(self, base, *parts):
        """Joins the given path part(s) to the given base_page path.
    ...
    def join_paths(self, base, *paths):
        """Joins given paths with base_page and returns resulted paths.

        See `Join Path` for more information.

        Examples:
        | @{p1} = | Join Paths | base_page     | example       | other |          |
        | @{p2} = | Join Paths | /my/base_page | /example      | other |          |
        | @{p3} = | Join Paths | my/base_page  | example/path/ | other | one/more |
        =>
        - @{p1} = ['base_page/example', 'base_page/other']
        - @{p2} = ['/example', '/my/base_page/other']
        - @{p3} = ['my/base_page/example/path', 'my/base_page/other', 'my/base_page/one/more']
        """
    ...
    def split_extension(self, path):
        ...
        removed). The base_page path and extension are returned as separate
